...
const Input = forwardRef((props, ref) => {
  const { isPassword = false, name, value, className, icon, disabled, rounded, onChange, onKeyPress, dropdown, fluid, big } = props;
  ...
  return (
    <Form.Input
      ...
      {isEmpty(dropdown) && (!isEmpty(icon) || isPassword) && <Icon name={isPassword ? "lock" : icon} />}
      <input ref={ref || input} />
      {!isEmpty(value) && !disabled && (
        <Button className={rounded ? styles.clearRoundedIconLast : styles.clearIconLast} icon="cancel" onClick={clearInput} />
      )}
      {isPassword && <Button className={styles.clearIconLast} icon={passwordIcon} onClick={handleToggle} />}
    </Form.Input>
  );
});
